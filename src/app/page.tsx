...
export default function HomePage() {
  return (
    <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden bg-rice">
          ...
          <div className="flex-1 relative w-full max-w-md lg:max-w-none">
            ...
            <div className="absolute -bottom-6 -left-6 bg-white p-4 rounded-xl shadow-xl border border-rice-deep z-20 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-ink">战国凤鸟纹</p>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Link href="/gallery" className="card p-8 group flex flex-col h-full">
                ...
                <p className="text-sm text-ink-light leading-relaxed mb-6 flex-1">浏览 1000+ 高清数字档案，支持按年代、地域、工艺多维检索。</p>
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-10">
            <div className="flex flex-col md:flex-row items-center justify-between mb-12 gap-6">
              <div>
                ...
                <p className="text-ink-light">实时监控学术平台核心指标</p>
              </div>
              <Link href="/dashboard" className="flex items-center gap-2 text-sm font-bold text-cinnabar hover:text-cinnabar-deep transition-colors bg-cinnabar/5 px-4 py-2 rounded-lg">
                查看完整数据报告
                <Icon name="arrow_forward" size={16} />
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="flex flex-col items-center justify-center p-10 bg-white rounded-2xl border border-rice-deep shadow-sm">
                ...
                <p className="text-5xl font-black text-ink tabular-nums mb-2">{mockStats.totalPatterns.toLocaleString()}</p>
                <p className="text-sm font-bold text-ink-light uppercase tracking-widest">总模式数量</p>
              </div>
              <div className="flex flex-col items-center justify-center p-10 bg-white rounded-2xl border border-rice-deep shadow-sm">
                <Icon name="group" size={40} className="text-cinnabar/40 mb-4" />
                <p className="text-5xl font-black text-ink tabular-nums mb-2">{mockStats.totalUsers.toLocaleString()}</p>
                <p className="text-sm font-bold text-ink-light uppercase tracking-widest">累计用户数</p>
              </div>
              <div className="flex flex-col items-center justify-center p-10 bg-white rounded-2xl border border-rice-deep shadow-sm">
                <Icon name="smart_toy" size={40} className="text-cinnabar/40 mb-4" />
                <p className="text-5xl font-black text-ink tabular-nums mb-2">{mockStats.aiCalls}</p>
                <p className="text-sm font-bold text-ink-light uppercase tracking-widest">AI 调用次数</p>
              </div>
            </div>
          </div>
        </section>
              ...
              {mockPatterns.slice(0, 4).map((p) => (
                <Link key={p.id} href={`/gallery/${p.id}`} className="flex flex-col gap-4 group">
                  <div className="aspect-[4/5] bg-white/5 rounded-xl overflow-hidden border border-white/10 relative">
                    ...
                    <div className="w-full h-full group-hover:scale-110 transition-transform duration-700" style={{ backgroundColor: p.imagePlaceholderColor }} />
                  ...
                  <div>
                    ...
                    <span className="text-sm text-white/60">{p.era} · {p.technique}</span>
                  </div>
                </Link>
              ))}
            </div>
      ...
      {/* Footer */}
      <footer className="bg-rice-warm py-12 px-6 lg:px-20">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-10">
            <div className="flex flex-col gap-4">
              ...
              <Link className="text-sm text-ink-light hover:text-cinnabar" href="/gallery">纹样分类</Link>
              <Link className="text-sm text-ink-light hover:text-cinnabar" href="/dashboard">学术文章</Link>
              <Link className="text-sm text-ink-light hover:text-cinnabar" href="#">关于我们</Link>
            </div>
            <div className="flex flex-col gap-4">
