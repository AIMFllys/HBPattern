...
graph TB
    ...
    subgraph "数据与存储层 (阿里云)"
        RDS["阿里云 RDS PostgreSQL<br/>+ PostGIS + pgvector"]
        OSS["阿里云 OSS<br/>对象存储"]
...
技术选型：
- **状态存储**：RDS PostgreSQL ai_tasks 表
...
> 4. **阿里云 RDS 原生支持**：同地域内网 <1ms 延迟，无需海外绕行
...
| **数据库** | 阿里云 RDS PostgreSQL（同地域内网 <1ms） |
| **对象存储** | 阿里云 OSS |
| **缓存** | 阿里云 Redis |
| **认证** | NextAuth.js |
...
graph TB
    ...
    subgraph "阿里云数据服务（同地域内网）"
        RDS["RDS PostgreSQL 16<br/>+ PostGIS + pgvector"]
...
| 服务 | 规格 | 预估月成本 | 说明 |
|------|------|:---:|------|
| RDS PostgreSQL | 基础版 2核4G, 20GB ESSD | ~30-50元 | 按量付费，闲置时仅收存储 |
...
| 模块 | 交付物 |
|------|--------|
| 基础架构 | Next.js 16.2 初始化 + RDS PostgreSQL 建库 + 18张表完整迁移 |
...
| 8 | 数据库 | **阿里云 RDS PostgreSQL** | MySQL / Supabase | 国内同地域内网<1ms+PostGIS |
